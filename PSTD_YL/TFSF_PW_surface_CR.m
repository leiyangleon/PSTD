...
zeta_x=ones(1,xdim);
% zeta_x=[zeros(1,bound_width_x) ibh_y ones(1,xdim-2*bound_width_x-2*ibh_th) fliplr(ibh_y) zeros(1,bound_width_x)];
% zeta_x=[zeros(1,bound_width_x) ones(1,xdim-2*bound_width_x) zeros(1,bound_width_x)];
...
% TFSF_location_T=(ibh_th+1):(xdim-ibh_th-1);
% TFSF_location_T=(bound_width_x+ibh_th-1):(xdim-bound_width_x-ibh_th+1);
% TFSF_location_T=(bound_width_x+1):(xdim-bound_width_x);
...
for n= 1:time_tot
    ...
    Ez_inc_T = sin(((2.*pi.*c)./(N_lambda_signal.*delta)).*(t_temp-TFSF_lag_tri_T-time_shift).*deltat).*...
        exp(-(((t_temp-TFSF_lag_tri_T-time_shift).*deltat).^2)./(0.966./BW)^2);
    
    Ez_inc_12_T = sin(((2.*pi.*c)./(N_lambda_signal.*delta)).*(t_temp+0.5-TFSF_lag_tri_T-time_shift).*deltat).*...
        exp(-(((t_temp+0.5-TFSF_lag_tri_T-time_shift).*deltat).^2)./(0.966./BW)^2);
